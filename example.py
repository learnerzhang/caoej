...
class KZPredictor:
    def __init__(self):
        ...
        with codecs.open('config/models_cfg.json', 'r', 'utf-8') as f:
            self.models = json.load(f)

        for model in self.models:
            self.models[model]['model'] = joblib.load(self.models[model]['model_path'])
            if 'model_encoder_path' in self.models[model]:
                self.models[model]['encoder'] = joblib.load(self.models[model]['model_encoder_path'])
    ...
    def predict_time(self, input_data, model=None):
        """预测开闸时间"""
    ...
    def predict_num(self, input_data, model=None):
        """预测开闸孔数"""
    ...
    def predict_level(self, input_data, model=None):
        """预测目标水位"""
    ...
    def predict_duration(self, input_data, model=None, encoder=None):
        """预测开闸时长类别"""
    ...
    def predict_comb(self, input_data, model=None):
        """预测开闸时长*开孔数量的log值"""
    ...
    def batch_predict(self, input_data):
        ...
        resultMap = collections.defaultdict(dict)
        for mkey in self.models:
            if mkey is None:
                continue
            model_name = self.models[mkey]['model_name']

            model = self.models[mkey]['model']
            model_tag = self.models[mkey]['model_tag']
            model_desc = self.models[mkey]['model_desc']
            model_label = self.models[mkey]['model_label']
            model_value = self.models[mkey]['model_value']

            encoder = self.models[mkey]['encoder'] if 'encoder' in self.models[mkey] else None
            if model_name == 'TimePredictor':
                value = self.predict_time(input_data, model).strftime("%Y-%m-%d %H:%M:00")
            elif model_name == 'NumPredictor':
                value = self.predict_num(input_data, model)
            elif model_name == 'LevelPredictor':
                value = self.predict_level(input_data, model)
            elif model_name == 'DuraPredictor':
                value = self.predict_duration(input_data, model, encoder)
            elif model_name == 'CombPredictor':
                self.predict_comb(input_data, model)
            else:
            ...
            if isinstance(model_value, list):
                ...
                label = 0
                for _label in range(6, 1, -1):
                    if 6 >= value / _label >=2:
                        label = _label
                        break
                
                if label > 0:
                    gate_count = self.LABEL_TO_GATES[label-1]
                    duration_in_hours = value / gate_count
                else:
                    gate_count = self.LABEL_TO_GATES[0]
                    duration_in_hours = value
                
                resultMap[model_tag].update({
...
def predict_all():
    ...
    # 创建示例输入数据
    with codecs.open('config/example.json', 'r', encoding='utf-8') as f:
...
def batch_predict():
    predictor = KZPredictor()
    # 创建示例输入数据
    with codecs.open('data/input_v3.json', 'r', encoding='utf-8') as f:
        example_input = json.load(f)
    # pprint.pprint(example_input)
    # 进行预测
    if isinstance(example_input['sign_time'], str):
        sign_time = datetime.strptime(example_input['sign_time'], '%Y-%m-%d %H:%M:%S')
        example_input['sign_time'] = sign_time
    predictions = predictor.batch_predict(example_input)
    return predictions
