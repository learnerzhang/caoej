...
def create_preprocessor():
    ...
    # 使用安全的分类特征处理器 - 修复OneHotEncoder参数
    categorical_transformer = Pipeline(steps=[
        ('imputer', SafeSimpleImputer(strategy='constant', fill_value=0)),
        ('onehot', OneHotEncoder(handle_unknown='ignore', sparse=False))  # 修复这里
    ])
...
def train_classification_model():
    ...
    try:
        ...
        plt.xlabel('预测孔数')
        plt.ylabel('实际孔数')
        plt.title('开闸孔数预测混淆矩阵')
        plt.tight_layout()
        plt.savefig('outputs/kz_num/plots/0822_kz_num_confusion_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # 保存模型
        os.makedirs('outputs/kz_num/pkls', exist_ok=True)
        joblib.dump(best_model, 'outputs/kz_num/pkls/0822_kz_num_classification_model.pkl')
        print("分类模型已保存到 outputs/kz_num/pkls/0822_kz_num_classification_model.pkl")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        os.makedirs('outputs/kz_num/outputs', exist_ok=True)
        results_df.to_csv('outputs/kz_num/outputs/0822_kz_num_grid_search_results.csv', index=False)
        ...
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_num/pkls/0822_kz_num_classification_model_default.pkl')
        print("默认分类模型已保存")
