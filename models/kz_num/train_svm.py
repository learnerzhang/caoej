...
def create_preprocessor():
    ...
    numeric_transformer = Pipeline(steps=[
        ...
        ('scaler', StandardScaler())  # SVM对特征缩放敏感，需要标准化
    ])
...
def analyze_errors(test_results, X_test, y_test, y_pred):
    ...
    if len(errors) > 0:
        ...
        # 保存错误分析结果
        error_pairs.to_csv('outputs/kz_num/outputs/0822_kz_num_svm_error_analysis.csv', index=False)
        ...
        plt.xlabel('真实孔数')
        plt.ylabel('错误率 (%)')
        plt.title('SVM模型各类别的错误率')
        plt.xticks(x_pos, error_rates.index)
        plt.tight_layout()
        plt.savefig('outputs/kz_num/plots/0822_kz_num_svm_error_rates.png', dpi=300, bbox_inches='tight')
        plt.close()
...
def train_classification_model():
    ...
    param_grid = {
        ...
        'classifier__degree': [2, 3, 4],  # 多项式核的度数
    }
    ...
    try:
        ...
        plt.xlabel('预测孔数')
        plt.ylabel('实际孔数')
        plt.title('SVM开闸孔数预测混淆矩阵')
        plt.tight_layout()
        plt.savefig('outputs/kz_num/plots/0822_kz_num_svm_confusion_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        try:
            ...
            importance_df.to_csv('outputs/kz_num/outputs/0822_kz_num_svm_feature_importance.csv', index=False)
            
            # 可视化最重要的特征
            plt.figure(figsize=(12, 8))
            sns.barplot(x='importance', y='feature', 
                        data=importance_df.head(20))
            plt.title('SVM开闸孔数预测模型特征重要性排名')
            plt.tight_layout()
            plt.savefig('outputs/kz_num/plots/0822_kz_num_svm_feature_importance.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        print("SVM分类模型已保存到 outputs/kz_num/pkls/0822_kz_num_svm_classification_model.pkl")
        
        # 保存网格搜索结果
        results = pd.DataFrame(grid_search.cv_results_)
        results.to_csv('outputs/kz_num/outputs/0822_kz_num_svm_grid_search_results.csv', index=False)
        ...
        test_results.to_csv('outputs/kz_num/outputs/test_predictions_svm_num.csv', index=False)
        print("测试集预测结果已保存")
        ...
        try:
            ...
            # 保存每日结果
            daily_results.to_csv('outputs/kz_num/outputs/daily_gate_count_svm_predictions.csv', index=False)
            print("每日开闸孔数预测结果已保存到 outputs/kz_num/outputs/daily_gate_count_svm_predictions.csv")
            ...
            plt.xlabel('日期')
            plt.ylabel('开闸孔数')
            plt.title('每日真实开闸孔数 vs 预测开闸孔数 (SVM)')
            plt.legend()
            plt.grid(True)
            plt.xticks(rotation=45)
            plt.tight_layout()
            plt.savefig('outputs/kz_num/plots/daily_gate_count_svm_comparison.png', dpi=300, bbox_inches='tight')
            plt.close()
            print("每日开闸孔数对比图已保存到 outputs/kz_num/plots/daily_gate_count_svm_comparison.png")
        ...
        print(f"默认SVM模型测试集准确率: {accuracy:.4f}")
        print(f"默认SVM模型测试集F1分数: {f1:.4f}")
        
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_num/pkls/0822_kz_num_svm_classification_model_default.pkl')
        print("默认SVM分类模型已保存")
