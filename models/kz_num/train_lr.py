...
def analyze_errors(test_results, X_test, y_test, y_pred):
    ...
    if len(errors) > 0:
        ...
        plt.tight_layout()
        plt.savefig('outputs/kz_num/plots/0822_kz_num_error_rates.png', dpi=300)
        plt.close()
...
def train_enhanced_classification_model():
    ...
    try:
        ...
        plt.xlabel('预测孔数')
        plt.ylabel('实际孔数')
        plt.title('优化逻辑回归开闸孔数预测混淆矩阵')
        plt.tight_layout()
        plt.savefig('outputs/kz_num/plots/0822_kz_num_lr_enhanced_confusion_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # 保存模型和结果
        joblib.dump(best_model, 'outputs/kz_num/pkls/0822_kz_num_lr_enhanced_classification_model.pkl')
        print("优化逻辑回归分类模型已保存")
        
        # 保存网格搜索结果
        results = pd.DataFrame(grid_search.cv_results_)
        results.to_csv('outputs/kz_num/outputs/0822_kz_num_lr_enhanced_grid_search_results.csv', index=False)
        
        # 返回测试集预测结果
        test_results = pd.DataFrame({
            'date': X_test['date'] if 'date' in X_test.columns else range(len(X_test)),
            'true_gate_count': y_test,
            'pred_gate_count': y_pred
        })
        test_results.to_csv('outputs/kz_num/outputs/test_predictions_lr_enhanced_num.csv', index=False)
        print("测试集预测结果已保存")
        ...
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_num/pkls/0822_kz_num_lr_enhanced_classification_model_default.pkl')
        print("默认逻辑回归分类模型已保存")
        
        return default_model
    
if __name__ == '__main__':
    model = train_enhanced_classification_model()
    print("逻辑回归模型训练完成！")