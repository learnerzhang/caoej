...
def train_regression_model():
    ...
    try:
        ...
        plt.xlabel('实际目标水位(米)')
        plt.ylabel('预测目标水位(米)')
        plt.title('SVM模型: 实际值 vs 预测值')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('outputs/kz_level/plots/0822_kz_level_svm_regression_scatter.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.xlabel('预测误差(米)')
        plt.ylabel('频率')
        plt.title('SVM模型: 目标水位预测误差分布')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('outputs/kz_level/plots/0822_kz_level_svm_error_distribution.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        try:
            ...
            importance_df.to_csv('outputs/kz_level/outputs/0822_kz_level_svm_regression_feature_importance.csv', index=False)
            
            # 可视化最重要的特征
            plt.figure(figsize=(12, 8))
            sns.barplot(x='importance', y='feature', 
                        data=importance_df.head(20))
            plt.title('SVM模型特征重要性排名 (基于排列重要性)')
            plt.tight_layout()
            plt.savefig('outputs/kz_level/plots/0822_kz_level_svm_regression_feature_importance.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        # 保存模型
        joblib.dump(best_model, 'outputs/kz_level/pkls/0822_kz_level_svm_regression_model.pkl')
        print("SVM模型已保存到 outputs/kz_level/pkls/0822_kz_level_svm_regression_model.pkl")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        results_df.to_csv('outputs/kz_level/outputs/0822_kz_level_svm_regression_grid_search_results.csv', index=False)
        ...
        if dates_test is not None:
            ...
            daily_results.to_csv('outputs/kz_level/outputs/0822_daily_water_level_svm_predictions.csv', index=False)
            print("每日目标水位预测结果已保存")
            ...
            plt.xlabel('日期')
            plt.ylabel('目标水位(米)')
            plt.title('SVM模型: 每日真实目标水位 vs 预测目标水位')
            plt.legend()
            plt.grid(True)
            plt.xticks(rotation=45)
            plt.tight_layout()
            plt.savefig('outputs/kz_level/plots/0822_daily_water_level_svm_comparison.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_level/pkls/0822_kz_level_svm_regression_model_default.pkl')
        print("默认SVM模型已保存")
