...
def train_regression_model():
    ...
    try:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('outputs/kz_level/plots/0822_kz_level_rf_regression_scatter.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.xlabel('预测误差(米)')
        plt.ylabel('频率')
        plt.title('RF模型: 目标水位预测误差分布')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('outputs/kz_level/plots/0822_kz_level_rf_error_distribution.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        try:
            ...
            if hasattr(regressor, 'feature_importances_'):
                ...
                importance_df.to_csv('outputs/kz_level/outputs/0822_kz_level_rf_regression_feature_importance.csv', index=False)
                ...
                plt.tight_layout()
                plt.savefig('outputs/kz_level/plots/0822_kz_level_rf_regression_feature_importance.png', dpi=300, bbox_inches='tight')
                plt.close()
        ...
        # 保存模型
        joblib.dump(best_model, 'outputs/kz_level/pkls/0822_kz_level_rf_regression_model.pkl')
        print("随机森林模型已保存到 outputs/kz_level/pkls/0822_kz_level_rf_regression_model.pkl")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        results_df.to_csv('outputs/kz_level/outputs/0822_kz_level_rf_regression_grid_search_results.csv', index=False)
        ...
        if dates_test is not None:
            ...
            daily_results.to_csv('outputs/kz_level/outputs/0822_daily_water_level_rf_predictions.csv', index=False)
            print("每日目标水位预测结果已保存")
            ...
            plt.xlabel('日期')
            plt.ylabel('目标水位(米)')
            plt.title('RF模型: 每日真实目标水位 vs 预测目标水位')
            plt.legend()
            plt.grid(True)
            plt.xticks(rotation=45)
            plt.tight_layout()
            plt.savefig('outputs/kz_level/plots/0822_daily_water_level_rf_comparison.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_level/pkls/0822_kz_level_rf_regression_model_default.pkl')
        print("默认随机森林模型已保存")
