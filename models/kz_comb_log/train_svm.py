...
def train_svm_model():
    ...
    try:
        ...
        print(f"最佳交叉验证MAE: {-grid_search.best_score_:.4f}")
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_svm_regression_scatter.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.xlabel('预测误差')
        plt.ylabel('频率')
        plt.title('SVM模型: 预测误差分布')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_svm_error_distribution.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # 保存模型
        os.makedirs('outputs/kz_comb_log/pkls', exist_ok=True)
        joblib.dump(best_model, 'outputs/kz_comb_log/pkls/0822_kz_comb_log_svm_regression_model.pkl')
        print("SVM模型已保存到 outputs/kz_comb_log/pkls/0822_kz_comb_log_svm_regression_model.pkl")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        os.makedirs('outputs/kz_comb_log/outputs', exist_ok=True)
        results_df.to_csv('outputs/kz_comb_log/outputs/0822_kz_comb_log_svm_grid_search_results.csv', index=False)
        ...
        if dates_test is not None:
            ...
            daily_results.to_csv('outputs/kz_comb_log/outputs/0822_daily_svm_predictions.csv', index=False)
            print("每日SVM预测结果已保存")
            ...
            plt.xlabel('日期')
            plt.ylabel('开闸时长×孔数')
            plt.title('SVM模型: 每日真实值 vs 预测值')
            plt.legend()
            plt.grid(True)
            plt.xticks(rotation=45)
            plt.tight_layout()
            plt.savefig('outputs/kz_comb_log/plots/0822_daily_svm_comparison.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        print(f"默认SVM模型测试集MAE: {mae:.4f}")
        print(f"默认SVM模型测试集R²: {r2:.4f}")
        
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_comb_log/pkls/0822_kz_comb_log_svm_regression_model_default.pkl')
        print("默认SVM模型已保存")
