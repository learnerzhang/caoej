...
def train_rf_model():
    ...
    try:
        ...
        for i, value in enumerate(y_targets[:, 5]):
            try:
                ...
                print(f"索引 {i} 的无效值: '{value}'，替换为中位数")
    ...
    model = Pipeline(steps=[
        ...
        ('regressor', RandomForestRegressor(random_state=42, n_jobs=n_jobs))
    ])
    ...
    try:
        ...
        print(f"最佳RF模型: {grid_search.best_estimator_}")
        ...
        plt.title('RF模型: 实际值 vs 预测值')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_rf_scatter.png', dpi=300)
        plt.close()
        ...
        plt.xlabel('预测误差')
        plt.ylabel('频率')
        plt.title('RF模型: 预测误差分布')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_rf_errors.png', dpi=300)
        plt.close()
        ...
        try:
            ...
            importance_df.to_csv('outputs/kz_comb_log/outputs/0822_kz_comb_log_rf_importance.csv', index=False)
            
            # 可视化特征重要性
            plt.figure(figsize=(12, 8))
            sns.barplot(x='importance', y='feature', data=importance_df.head(20))
            plt.title('RF模型: 特征重要性排名')
            plt.tight_layout()
            plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_rf_importance.png', dpi=300)
            plt.close()
        ...
        joblib.dump(best_model, 'outputs/kz_comb_log/pkls/0822_kz_comb_log_rf_model.pkl')
        print("RF模型已保存到 outputs/kz_comb_log/pkls/0822_kz_comb_log_rf_model.pkl")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        os.makedirs('outputs/kz_comb_log/outputs', exist_ok=True)
        results_df.to_csv('outputs/kz_comb_log/outputs/0822_kz_comb_log_rf_grid_results.csv', index=False)
        ...
        if dates_test is not None:
            ...
            daily_results.to_csv('outputs/kz_comb_log/outputs/0822_daily_comb_log_rf_predictions.csv', index=False)
            print("每日预测结果已保存")
            
            # 可视化每日对比
            plt.figure(figsize=(14, 7))
            plt.plot(pd.to_datetime(daily_results['date']), daily_results['true_comb_log'], 'o-', label='真实值')
            plt.plot(pd.to_datetime(daily_results['date']), daily_results['pred_comb_log'], 's--', label='预测值')
            plt.xlabel('日期')
            plt.ylabel('开闸时长×孔数')
            plt.title('RF模型: 每日真实值 vs 预测值')
            plt.legend()
            plt.grid(True)
            plt.xticks(rotation=45)
            plt.tight_layout()
            plt.savefig('outputs/kz_comb_log/plots/0822_daily_comb_log_rf_comparison.png', dpi=300)
            plt.close()
        ...
        print(f"默认模型MAE: {mae:.4f}")
        print(f"默认模型R²: {r2:.4f}")
        
        joblib.dump(default_model, 'outputs/kz_comb_log/pkls/0822_kz_comb_log_rf_model_default.pkl')
        print("默认RF模型已保存")
