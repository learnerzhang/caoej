...
def train_lr_model():
    ...
    try:
        ...
        print(f"最佳LR模型: {grid_search.best_estimator_}")
        ...
        plt.title('LR模型: 开闸时长×孔数预测 - 实际值 vs 预测值')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_lr_regression_scatter.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.xlabel('预测误差')
        plt.ylabel('频率')
        plt.title('LR模型: 开闸时长×孔数预测误差分布')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_lr_error_distribution.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        try:
            ...
            importance_df.to_csv('outputs/kz_comb_log/outputs/0822_kz_comb_log_lr_coefficients.csv', index=False)
            
            # 可视化最重要的特征（基于系数绝对值）
            plt.figure(figsize=(12, 8))
            sns.barplot(x='abs_importance', y='feature', 
                        data=importance_df.head(20))
            plt.title('LR模型特征重要性（基于系数绝对值）')
            plt.tight_layout()
            plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_lr_feature_importance.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        joblib.dump(best_model, 'outputs/kz_comb_log/pkls/0822_kz_comb_log_lr_regression_model.pkl')
        print("LR回归模型已保存到 outputs/kz_comb_log/pkls/0822_kz_comb_log_lr_regression_model.pkl")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        os.makedirs('outputs/kz_comb_log/outputs', exist_ok=True)
        results_df.to_csv('outputs/kz_comb_log/outputs/0822_kz_comb_log_lr_regression_grid_search_results.csv', index=False)
        ...
        if dates_test is not None:
            ...
            daily_results.to_csv('outputs/kz_comb_log/outputs/0822_daily_comb_log_lr_predictions.csv', index=False)
            print("每日开闸时长×孔数预测结果已保存")
            ...
            plt.xlabel('日期')
            plt.ylabel('开闸时长×孔数')
            plt.title('LR模型: 每日真实开闸时长×孔数 vs 预测开闸时长×孔数')
            plt.legend()
            plt.grid(True)
            plt.xticks(rotation=45)
            plt.tight_layout()
            plt.savefig('outputs/kz_comb_log/plots/0822_daily_comb_log_lr_comparison.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        print(f"默认LR模型测试集MAE: {mae:.4f}")
        print(f"默认LR模型测试集R²: {r2:.4f}")
        
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_comb_log/pkls/0822_kz_comb_log_lr_regression_model_default.pkl')
        print("默认LR回归模型已保存")
