...
def data_quality_check(X, y):
    ...
    for feature in categorical_features:
        if feature in X.columns:
            logger.info(f"\n{feature} 的值分布:")
            logger.info(X[feature].value_counts())
    
    # 检查目标变量的分布
    logger.info(f"\n目标变量统计:")
    logger.info(f"最小值: {np.min(y):.4f}")
    logger.info(f"最大值: {np.max(y):.4f}")
    logger.info(f"平均值: {np.mean(y):.4f}")
    logger.info(f"标准差: {np.std(y):.4f}")
    logger.info(f"中位数: {np.median(y):.4f}")
    ...
    logger.info(f"异常值数量: {len(outliers)} ({len(outliers)/len(y)*100:.2f}%)")
    
    return X, y

def train_comb_log_model():
    """训练开闸时长*孔数回归模型"""
    try:
        # 加载特征数据
        ...
        try:
            ...
            logger.warning(f"开闸时长*孔数 数据转换错误: {e}")
            # 处理非数值数据
            valid_targets = []
            for i, value in enumerate(y_targets[:, 5]):
                try:
                    ...
                    logger.warning(f"索引 {i} 的无效开闸时长*孔数值: '{value}'，替换为中位数")
                    # 使用非异常值的中位数替换
        ...
        logger.info(f"目标变量范围: {np.min(y):.2f}-{np.max(y):.2f}")
        logger.info(f"目标变量均值: {np.mean(y):.2f}, 标准差: {np.std(y):.2f}")
        ...
        logger.info("开始网格搜索寻找最佳回归模型...")
        
        grid_search.fit(X_train, y_train)
        
        logger.info(f"最佳模型: {grid_search.best_estimator_}")
        logger.info(f"最佳MAE: {-grid_search.best_score_:.4f}")
        ...
        plt.title('开闸时长×孔数预测: 实际值 vs 预测值')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_regression_scatter.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.xlabel('预测误差')
        plt.ylabel('频率')
        plt.title('开闸时长×孔数预测误差分布')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_error_distribution.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        try:
            ...
            if hasattr(regressor, 'feature_importances_'):
                ...
                importance_df.to_csv('outputs/kz_comb_log/outputs/0822_kz_comb_log_regression_feature_importance.csv', index=False)
                
                # 可视化最重要的特征
                plt.figure(figsize=(12, 8))
                sns.barplot(x='importance', y='feature', 
                            data=importance_df.head(20))
                plt.title('开闸时长×孔数预测模型特征重要性排名')
                plt.tight_layout()
                plt.savefig('outputs/kz_comb_log/plots/0822_kz_comb_log_regression_feature_importance.png', dpi=300, bbox_inches='tight')
                plt.close()
                
        except Exception as e:
            logger.error(f"无法获取特征重要性: {str(e)}")
        
        # 保存模型
        os.makedirs('outputs/kz_comb_log/pkls', exist_ok=True)
        joblib.dump(best_model, 'outputs/kz_comb_log/pkls/0822_kz_comb_log_regression_model.pkl')
        logger.info("回归模型已保存到 outputs/kz_comb_log/pkls/0822_kz_comb_log_regression_model.pkl")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        os.makedirs('outputs/kz_comb_log/outputs', exist_ok=True)
        results_df.to_csv('outputs/kz_comb_log/outputs/0822_kz_comb_log_regression_grid_search_results.csv', index=False)
        ...
        if dates_test is not None:
            ...
            daily_results.to_csv('outputs/kz_comb_log/outputs/0822_daily_comb_log_predictions.csv', index=False)
            logger.info("每日开闸时长×孔数预测结果已保存")
            ...
            plt.xlabel('日期')
            plt.ylabel('开闸时长×孔数')
            plt.title('每日真实开闸时长×孔数 vs 预测开闸时长×孔数')
            plt.legend()
            plt.grid(True)
            plt.xticks(rotation=45)
            plt.tight_layout()
            plt.savefig('outputs/kz_comb_log/plots/0822_daily_comb_log_comparison.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        logger.info(f"默认模型测试集MAE: {mae:.4f}")
        logger.info(f"默认模型测试集R²: {r2:.4f}")
        
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_comb_log/pkls/0822_kz_comb_log_regression_model_default.pkl')
        logger.info("默认回归模型已保存")
...
if __name__ == '__main__':
    ...
    logger.info("开始训练开闸时长×孔数回归模型...")
    model = train_comb_log_model()
    logger.info("训练完成！")