...
def train_and_save_model():
    ...
    random_search = RandomizedSearchCV(
        ...
        n_jobs=1,   # XGBoost已经使用了n_jobs，这里设为1避免冲突
        verbose=1,
        random_state=1122
    )
    ...
    try:
        ...
        # 添加参考线（y=x）
        min_val = min(y_test.min(), y_pred_hours.min())
        max_val = max(y_test.max(), y_pred_hours.max())
        plt.plot([min_val, max_val], [min_val, max_val], 'r--')
        
        plt.xlabel('实际开闸时间(小时)')
        plt.ylabel('预测开闸时间(小时)')
        plt.title('实际值 vs 预测值 (XGBoost模型)')
        plt.grid(True)
        plt.savefig('outputs/kz_time/plots/0822_kz_time_xgb_regression_scatter.png')
        plt.close()
        ...
        plt.grid(True)
        plt.savefig('outputs/kz_time/plots/0822_kz_time_xgb_error_distribution.png')
        plt.close()
        
        # 特征重要性分析 - XGBoost有自己的特征重要性
        try:
            # 获取特征名称
            feature_names = []
            ...
            importance_df.to_csv('outputs/kz_time/outputs/0822_kz_time_xgb_regression_feature_importance.csv', index=False)
            ...
            plt.tight_layout()
            plt.savefig('outputs/kz_time/plots/0822_kz_time_xgb_regression_feature_importance.png')
            plt.close()
        ...
        # 保存整个模型管道
        joblib.dump(best_model, 'outputs/kz_time/pkls/0822_kz_time_xgb_regression_model.pkl')
        
        # 单独保存预处理器和回归器以便后续使用
        joblib.dump(preprocessor, 'outputs/kz_time/pkls/0822_kz_time_xgb_preprocessor.pkl')
        joblib.dump(regressor, 'outputs/kz_time/pkls/0822_kz_time_xgb_regressor.pkl')
        
        print("XGBoost回归模型已保存到 outputs/kz_time/pkls/0822_kz_time_xgb_regression_model.pkl")
        print("预处理器已保存到 outputs/kz_time/pkls/0822_kz_time_xgb_preprocessor.pkl")
        print("回归器已保存到 outputs/kz_time/pkls/0822_kz_time_xgb_regressor.pkl")
        
        # 保存随机搜索结果
        results = pd.DataFrame(random_search.cv_results_)
        results.to_csv('outputs/kz_time/outputs/0822_kz_time_xgb_regression_random_search_results.csv', index=False)
        ...
        daily_results[['date', 'true_time', 'pred_time', 'error']].to_csv(
            'outputs/kz_time/outputs/daily_open_time_xgb_predictions.csv', index=False)

        print("\n每日开闸时间预测结果已保存到 outputs/kz_time/outputs/daily_open_time_xgb_predictions.csv")
        ...
        plt.xlabel('日期')
        plt.ylabel('开闸时间(小时)')
        plt.title('每日真实开闸时间 vs 预测开闸时间 (XGBoost模型)')
        plt.legend()
        plt.grid(True)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig('outputs/kz_time/plots/daily_time_xgb_comparison.png')
        plt.close()
        ...
        mae = mean_absolute_error(y_test, y_pred_hours)
        print(f"默认XGBoost模型测试集MAE: {mae:.4f}小时")
        
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_time/pkls/0822_kz_time_xgb_regression_model_default.pkl')
        print("默认XGBoost回归模型已保存")
