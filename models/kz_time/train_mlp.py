...
def train_and_save_model():
    ...
    try:
        ...
        plt.title('实际值 vs 预测值 (MLP模型)')
        plt.grid(True)
        plt.savefig('outputs/kz_time/plots/0822_kz_time_mlp_regression_scatter.png')
        plt.close()
        ...
        plt.xlabel('预测误差(小时)')
        plt.ylabel('频率')
        plt.title('MLP模型预测误差分布')
        plt.grid(True)
        plt.savefig('outputs/kz_time/plots/0822_kz_time_mlp_error_distribution.png')
        plt.close()
        
        # 特征重要性分析 - 使用随机森林作为代理模型
        try:
            # 获取特征名称
            feature_names = []
            ...
            importance_df.to_csv('outputs/kz_time/outputs/0822_kz_time_mlp_regression_feature_importance.csv', index=False)
            
            # 可视化最重要的特征
            plt.figure(figsize=(12, 8))
            sns.barplot(x='importance', y='feature', 
                        data=importance_df.head(20))
            plt.title('MLP模型特征重要性排名 (基于代理模型)')
            plt.tight_layout()
            plt.savefig('outputs/kz_time/plots/0822_kz_time_mlp_regression_feature_importance.png')
            plt.close()
        ...
        # 保存整个模型管道
        joblib.dump(best_model, 'outputs/kz_time/pkls/0822_kz_time_mlp_regression_model.pkl')
        
        # 单独保存预处理器和回归器以便后续使用
        joblib.dump(preprocessor, 'outputs/kz_time/pkls/0822_kz_time_mlp_preprocessor.pkl')
        joblib.dump(regressor, 'outputs/kz_time/pkls/0822_kz_time_mlp_regressor.pkl')
        
        print("MLP回归模型已保存到 outputs/kz_time/pkls/0822_kz_time_mlp_regression_model.pkl")
        print("预处理器已保存到 outputs/kz_time/pkls/0822_kz_time_mlp_preprocessor.pkl")
        print("回归器已保存到 outputs/kz_time/pkls/0822_kz_time_mlp_regressor.pkl")
        
        # 保存网格搜索结果
        results = pd.DataFrame(grid_search.cv_results_)
        results.to_csv('outputs/kz_time/outputs/0822_kz_time_mlp_regression_grid_search_results.csv', index=False)
        ...
        daily_results[['date', 'true_time', 'pred_time', 'error']].to_csv(
            'outputs/kz_time/outputs/daily_open_time_mlp_predictions.csv', index=False)

        print("\n每日开闸时间预测结果已保存到 outputs/kz_time/outputs/daily_open_time_mlp_predictions.csv")
        ...
        plt.xlabel('日期')
        plt.ylabel('开闸时间(小时)')
        plt.title('每日真实开闸时间 vs 预测开闸时间 (MLP模型)')
        plt.legend()
        plt.grid(True)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig('outputs/kz_time/plots/daily_time_mlp_comparison.png')
        plt.close()
        ...
        mae = mean_absolute_error(y_test, y_pred_hours)
        print(f"默认MLP模型测试集MAE: {mae:.4f}小时")
        
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_time/pkls/0822_kz_time_mlp_regression_model_default.pkl')
        print("默认MLP模型已保存")
