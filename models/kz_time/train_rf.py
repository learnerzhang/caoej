...
def create_preprocessor():
    ...
    # 使用安全的数值型转换器 - 随机森林不需要标准化，但保留以保持一致性
...
def train_and_save_model():
    ...
    try:
        ...
        plt.title('实际值 vs 预测值 (随机森林模型)')
        plt.grid(True)
        plt.savefig('outputs/kz_time/plots/0822_kz_time_rf_regression_scatter.png')
        plt.close()
        ...
        plt.xlabel('预测误差(小时)')
        plt.ylabel('频率')
        plt.title('随机森林模型预测误差分布')
        plt.grid(True)
        plt.savefig('outputs/kz_time/plots/0822_kz_time_rf_error_distribution.png')
        plt.close()
        
        # 特征重要性分析 - 随机森林有自己的特征重要性
        try:
            # 获取特征名称
            ...
            importance_df.to_csv('outputs/kz_time/outputs/0822_kz_time_rf_regression_feature_importance.csv', index=False)
            ...
            plt.tight_layout()
            plt.savefig('outputs/kz_time/plots/0822_kz_time_rf_regression_feature_importance.png')
            plt.close()
        ...
        # 保存模型
        os.makedirs('outputs/kz_time/pkls', exist_ok=True)
        joblib.dump(best_model, 'outputs/kz_time/pkls/0822_kz_time_rf_regression_model.pkl')
        print("随机森林回归模型已保存到 outputs/kz_time/pkls/0822_kz_time_rf_regression_model.pkl")
        
        # 保存网格搜索结果
        os.makedirs('outputs/kz_time/outputs', exist_ok=True)
        results = pd.DataFrame(grid_search.cv_results_)
        results.to_csv('outputs/kz_time/outputs/0822_kz_time_rf_regression_grid_search_results.csv', index=False)
        ...
        daily_results[['date', 'true_time', 'pred_time', 'error']].to_csv(
            'outputs/kz_time/outputs/daily_open_time_rf_predictions.csv', index=False)

        print("\n每日开闸时间预测结果已保存到 outputs/kz_time/outputs/daily_open_time_rf_predictions.csv")
        ...
        plt.xlabel('日期')
        plt.ylabel('开闸时间(小时)')
        plt.title('每日真实开闸时间 vs 预测开闸时间 (随机森林模型)')
        plt.legend()
        plt.grid(True)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig('outputs/kz_time/plots/daily_time_rf_comparison.png')
        plt.close()
        ...
        mae = mean_absolute_error(y_test, y_pred_hours)
        print(f"默认随机森林模型测试集MAE: {mae:.4f}小时")
        
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_time/pkls/0822_kz_time_rf_regression_model_default.pkl')
        print("默认随机森林回归模型已保存")
