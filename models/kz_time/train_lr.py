...
def train_and_save_model():
    ...
    param_grid = [
        ...
        },
        {
            ...
            'regressor__estimator__fit_intercept': [True, False]
        }
    ]
    ...
    try:
        ...
        plt.title('实际值 vs 预测值 (线性回归模型)')
        plt.grid(True)
        plt.savefig('outputs/kz_time/plots/0822_kz_time_lr_regression_scatter.png')
        plt.close()
        ...
        plt.xlabel('预测误差(小时)')
        plt.ylabel('频率')
        plt.title('线性回归模型预测误差分布')
        plt.grid(True)
        plt.savefig('outputs/kz_time/plots/0822_kz_time_lr_error_distribution.png')
        plt.close()
        
        # 特征重要性分析 - 对于线性模型，使用系数绝对值作为重要性
        try:
            # 获取特征名称
            feature_names = []
            ...
            if importances is not None:
                ...
                importance_df.to_csv('outputs/kz_time/outputs/0822_kz_time_lr_regression_feature_importance.csv', index=False)
                
                # 可视化最重要的特征
                plt.figure(figsize=(12, 8))
                sns.barplot(x='importance', y='feature', 
                            data=importance_df.head(20))
                plt.title('线性回归模型特征重要性排名')
                plt.tight_layout()
                plt.savefig('outputs/kz_time/plots/0822_kz_time_lr_regression_feature_importance.png')
                plt.close()
        ...
        # 保存整个模型管道
        joblib.dump(best_model, 'outputs/kz_time/pkls/0822_kz_time_lr_regression_model.pkl')
        
        # 单独保存预处理器和回归器以便后续使用
        joblib.dump(preprocessor, 'outputs/kz_time/pkls/0822_kz_time_lr_preprocessor.pkl')
        joblib.dump(regressor, 'outputs/kz_time/pkls/0822_kz_time_lr_regressor.pkl')
        
        print("回归模型已保存到 outputs/kz_time/pkls/0822_kz_time_lr_regression_model.pkl")
        print("预处理器已保存到 outputs/kz_time/pkls/0822_kz_time_lr_preprocessor.pkl")
        print("回归器已保存到 outputs/kz_time/pkls/0822_kz_time_lr_regressor.pkl")
        
        # 保存网格搜索结果
        results = pd.DataFrame(grid_search.cv_results_)
        results.to_csv('outputs/kz_time/outputs/0822_kz_time_lr_regression_grid_search_results.csv', index=False)
        ...
        daily_results[['date', 'true_time', 'pred_time', 'error']].to_csv(
            'outputs/kz_time/outputs/daily_open_time_lr_predictions.csv', index=False)

        print("\n每日开闸时间预测结果已保存到 outputs/kz_time/outputs/daily_open_time_lr_predictions.csv")
        ...
        plt.xlabel('日期')
        plt.ylabel('开闸时间(小时)')
        plt.title('每日真实开闸时间 vs 预测开闸时间 (线性回归)')
        plt.legend()
        plt.grid(True)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig('outputs/kz_time/plots/daily_time_lr_comparison.png')
        plt.close()
        ...
        mae = mean_absolute_error(y_test, y_pred_hours)
        print(f"默认线性回归模型测试集MAE: {mae:.4f}小时")
        
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_time/pkls/0822_kz_time_lr_regression_model_default.pkl')
        print("默认线性回归模型已保存")
