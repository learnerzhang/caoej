...
from sklearn.base import BaseEstimator, TransformerMixin  # 添加这行导入
...
def create_enhanced_preprocessor():
    ...
    numeric_features = [
        ...
        # 增强特征
        'flow_tide_interaction', 'rain_water_interaction', 'is_potential_long_op'
    ]
...
def train_svm_classification_model_with_class7_focus():
    """训练SVM开闸时长分类模型，特别关注类别7"""
    # 创建输出目录
    os.makedirs('outputs/kz_dura_svm_class7/plots', exist_ok=True)
    os.makedirs('outputs/kz_dura_svm_class7/pkls', exist_ok=True)
    os.makedirs('outputs/kz_dura_svm_class7/outputs', exist_ok=True)
    ...
    param_grid = {
        ...
        'classifier__kernel': ['rbf', 'linear'],  # 优先选择表现较好的核函数
    ...
    # 使用分层K折交叉验证
    cv_strategy = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)  # SVM计算成本高，使用3折
    ...
    try:
        ...
        plt.xlabel('预测时长')
        plt.ylabel('实际时长')
        plt.title('SVM开闸时长预测混淆矩阵（类别7优化）')
        plt.tight_layout()
        plt.savefig('outputs/kz_dura_svm_class7/plots/0822_kz_dura_svm_class7_confusion_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        try:
            ...
            importance_df.to_csv('outputs/kz_dura_svm_class7/outputs/0822_kz_dura_svm_class7_feature_importance.csv', index=False)
            
            # 可视化最重要的特征
            plt.figure(figsize=(12, 8))
            sns.barplot(x='importance', y='feature', data=importance_df.head(20))
            plt.title('SVM开闸时长分类模型特征重要性排名（类别7优化）')
            plt.tight_layout()
            plt.savefig('outputs/kz_dura_svm_class7/plots/0822_kz_dura_svm_class7_feature_importance.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        # 保存模型和标签编码器
        joblib.dump(best_model, 'outputs/kz_dura_svm_class7/pkls/0822_kz_dura_svm_class7_classification_model.pkl')
        joblib.dump(label_encoder, 'outputs/kz_dura_svm_class7/pkls/0822_kz_dura_svm_class7_label_encoder.pkl')
        print("SVM分类模型已保存到 outputs/kz_dura_svm_class7/pkls/0822_kz_dura_svm_class7_classification_model.pkl")
        print("标签编码器已保存到 outputs/kz_dura_svm_class7/pkls/0822_kz_dura_svm_class7_label_encoder.pkl")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        results_df.to_csv('outputs/kz_dura_svm_class7/outputs/0822_kz_dura_svm_class7_grid_search_results.csv', index=False)
        ...
        if len(class7_results) > 0:
            ...
            # 保存类别7详细预测结果
            class7_results.to_csv('outputs/kz_dura_svm_class7/outputs/0822_svm_class7_detailed_predictions.csv', index=False)
        
        test_results.to_csv('outputs/kz_dura_svm_class7/outputs/0822_test_svm_class7_predictions.csv', index=False)
        print("测试集预测结果已保存")
        ...
        print(f"默认模型类别7的F1分数: {class7_f1:.4f}")
        
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_dura_svm_class7/pkls/0822_kz_dura_svm_class7_classification_model_default.pkl')
        joblib.dump(label_encoder, 'outputs/kz_dura_svm_class7/pkls/0822_kz_dura_svm_class7_label_encoder_default.pkl')
        print("默认SVM分类模型已保存")
        
        return default_model, label_encoder

if __name__ == '__main__':
    model, encoder = train_svm_classification_model_with_class7_focus()
    print("SVM开闸时长分类模型（类别7优化）训练完成！")