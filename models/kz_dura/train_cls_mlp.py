...
def train_mlp_classification_model_with_class7_focus():
    """训练MLP开闸时长分类模型，特别关注类别7"""
    # 创建输出目录
    os.makedirs('outputs/kz_dura_class7_mlp/plots', exist_ok=True)
    os.makedirs('outputs/kz_dura_class7_mlp/pkls', exist_ok=True)
    os.makedirs('outputs/kz_dura_class7_mlp/outputs', exist_ok=True)
    ...
    param_grid = {
        ...
        'classifier__base_estimator__alpha': [0.0001, 0.001, 0.01],  # 正则化参数
        'classifier__base_estimator__learning_rate_init': [0.001, 0.01],
        'classifier__base_estimator__batch_size': [32, 64]  # 批处理大小
    }
    ...
    try:
        ...
        plt.xlabel('预测时长')
        plt.ylabel('实际时长')
        plt.title('MLP开闸时长预测混淆矩阵（类别7优化）')
        plt.tight_layout()
        plt.savefig('outputs/kz_dura_class7_mlp/plots/0822_kz_dura_class7_mlp_confusion_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        try:
            ...
            importance_df.to_csv('outputs/kz_dura_class7_mlp/outputs/0822_kz_dura_class7_mlp_feature_importance.csv', index=False)
            
            # 可视化最重要的特征
            plt.figure(figsize=(12, 8))
            sns.barplot(x='importance', y='feature', data=importance_df.head(20))
            plt.title('MLP开闸时长分类模型特征重要性排名（类别7优化）')
            plt.tight_layout()
            plt.savefig('outputs/kz_dura_class7_mlp/plots/0822_kz_dura_class7_mlp_feature_importance.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        # 保存模型和标签编码器
        joblib.dump(best_model, 'outputs/kz_dura_class7_mlp/pkls/0822_kz_dura_class7_mlp_classification_model.pkl')
        joblib.dump(label_encoder, 'outputs/kz_dura_class7_mlp/pkls/0822_kz_dura_class7_mlp_label_encoder.pkl')
        print("MLP分类模型已保存到 outputs/kz_dura_class7_mlp/pkls/0822_kz_dura_class7_mlp_classification_model.pkl")
        print("标签编码器已保存到 outputs/kz_dura_class7_mlp/pkls/0822_kz_dura_class7_mlp_label_encoder.pkl")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        results_df.to_csv('outputs/kz_dura_class7_mlp/outputs/0822_kz_dura_class7_mlp_grid_search_results.csv', index=False)
        ...
        if len(class7_results) > 0:
            ...
            # 保存类别7详细预测结果
            class7_results.to_csv('outputs/kz_dura_class7_mlp/outputs/0822_class7_mlp_detailed_predictions.csv', index=False)
        
        test_results.to_csv('outputs/kz_dura_class7_mlp/outputs/0822_test_class7_mlp_predictions.csv', index=False)
        print("测试集预测结果已保存")
        ...
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_dura_class7_mlp/pkls/0822_kz_dura_class7_mlp_classification_model_default.pkl')
        joblib.dump(label_encoder, 'outputs/kz_dura_class7_mlp/pkls/0822_kz_dura_class7_mlp_label_encoder_default.pkl')
        print("默认MLP分类模型已保存")
        
        return default_model, label_encoder

if __name__ == '__main__':
    model, encoder = train_mlp_classification_model_with_class7_focus()
    print("MLP开闸时长分类模型（类别7优化）训练完成！")