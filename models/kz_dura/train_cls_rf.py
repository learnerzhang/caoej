...
def create_class7_enhanced_features(X, y_duration):
    ...
    # 添加潮汐和流量的综合特征
    if 'tide_24h_range' in X.columns and 'flow_range' in X.columns:
        X_enhanced['tide_flow_variability'] = X['tide_24h_range'] * X['flow_range']
...
def create_enhanced_preprocessor():
    ...
    numeric_features = [
        ...
        # 增强特征
        'flow_tide_interaction', 'rain_water_interaction', 
        'is_potential_long_op', 'tide_flow_variability',
...
def train_optimized_rf_classification_model():
    """训练优化的随机森林分类模型"""
    # 创建输出目录
    os.makedirs('outputs/kz_dura/plots', exist_ok=True)
    os.makedirs('outputs/kz_dura/pkls', exist_ok=True)
    os.makedirs('outputs/kz_dura/outputs', exist_ok=True)
    ...
    # 对长时间类别给予额外权重
    long_duration_classes = [cls for cls in classes if label_encoder.inverse_transform([cls])[0] >= 5]
    ...
    try:
        ...
        plt.xlabel('预测时长')
        plt.ylabel('实际时长')
        plt.title('开闸时长预测混淆矩阵（优化随机森林）')
        plt.tight_layout()
        plt.savefig('outputs/kz_dura/plots/0822_kz_dura_rf_optimized_confusion_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        try:
            ...
            if hasattr(classifier, 'feature_importances_'):
                ...
                importance_df.to_csv('outputs/kz_dura/outputs/0822_kz_dura_rf_optimized_feature_importance.csv', index=False)
                
                # 可视化最重要的特征
                plt.figure(figsize=(12, 8))
                sns.barplot(x='importance', y='feature', data=importance_df.head(20))
                plt.title('优化随机森林分类模型特征重要性排名')
                plt.tight_layout()
                plt.savefig('outputs/kz_dura/plots/0822_kz_dura_rf_optimized_feature_importance.png', dpi=300, bbox_inches='tight')
                plt.close()
        ...
        # 保存模型和标签编码器
        joblib.dump(best_model, 'outputs/kz_dura/pkls/0822_kz_dura_rf_optimized_model.pkl')
        joblib.dump(label_encoder, 'outputs/kz_dura/pkls/0822_kz_dura_rf_optimized_label_encoder.pkl')
        print("优化随机森林分类模型已保存")
        
        # 保存网格搜索结果
        results_df = pd.DataFrame(grid_search.cv_results_)
        results_df.to_csv('outputs/kz_dura/outputs/0822_kz_dura_rf_optimized_grid_search_results.csv', index=False)
        ...
        if len(class7_results) > 0:
            ...
            # 保存类别7详细预测结果
            class7_results.to_csv('outputs/kz_dura/outputs/0822_rf_class7_detailed_predictions.csv', index=False)
        
        test_results.to_csv('outputs/kz_dura/outputs/0822_rf_optimized_predictions.csv', index=False)
        print("测试集预测结果已保存")
        ...
        # 保存默认模型
        joblib.dump(default_model, 'outputs/kz_dura/pkls/0822_kz_dura_rf_optimized_model_default.pkl')
        joblib.dump(label_encoder, 'outputs/kz_dura/pkls/0822_kz_dura_rf_optimized_label_encoder_default.pkl')
        print("默认优化随机森林模型已保存")
        
        return default_model, label_encoder

if __name__ == '__main__':
    model, encoder = train_optimized_rf_classification_model()
    print("优化的随机森林分类模型训练完成！")